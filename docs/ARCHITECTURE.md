...
17. Team Manager sends Codex the authoritative Sleeper starters and bench, slot eligibility, inferred available-player evidence, data readiness, matchup state, and league activity. Codex makes the lineup, waiver, drop, and roster-priority judgments; deterministic code checks coverage and preserves raw facts rather than choosing a winner.
...
25. Authenticated data-management routes expose aggregate counts, category deletion, a typed-confirmation reset, and a support report that reduces decision history to non-identifying event metadata.
