...
Each lookup uses a separate ephemeral Codex process/thread with live hosted search restricted to NFL.com and ESPN. At most two lookups are allowed per answer, each with a 20-second budget that includes process startup; the main turn retains its existing timeout. Closing or failing the parent turn closes research clients. Unsupported search configuration, failed retrieval, missing citations, and invalid source links return a safe evidence gap so Codex can continue. Source structure and URL allowlisting do not prove that an article or claim is accurate.

Copy diagnostics and support reports include the latest 20 lookup outcomes and elapsed milliseconds for the current provider session. Outcomes distinguish `available`, `timeout`, `cancelled`, `provider_error`, `invalid_json`, `invalid_response`, `invalid_source`, `invalid_date`, and `no_recent_sources`. An empty list means no lookup completed in that session; it does not prove search succeeded. Diagnostics retain no player identity, prompt, article, URL, or raw provider error and never launch Codex just to inspect the list.
...
Approved source hosts are exactly `nfl.com`, `www.nfl.com`, `amp.nfl.com`, `espn.com`, and `www.espn.com`. The [NFL AMP news site](https://amp.nfl.com/news/) is included because live search returns its articles. This is not a wildcard subdomain allowance; credentials, non-HTTPS URLs, and nonstandard ports remain rejected in source validation and Electron's external-link handler.

## AI-first Team Manager

Team Manager follows the same ownership rule as the draft assistant: deterministic code supplies evidence and safety facts, while Codex owns strategic judgment. The model receives the current Sleeper starters and bench, slot eligibility, roster counts, current matchup state, league activity, readiness warnings, and inferred available players. Available players are grouped by separate raw weekly-projection, ROS ECR, draft-ECR-fallback, and positional-retrieval signals; the catalog is alphabetical and no composite score or local winner is supplied.

The local engine does not choose lineup swaps, waiver additions, drops, weakest positions, or roster priorities. Codex must verify that a proposed starter is rostered and slot-eligible, that an addition appears in the inferred available-player evidence, and that a proposed drop is on the user's roster. Missing or partial weekly and season-value data must lower confidence rather than triggering rank-based fallback advice. Draft ECR fallback is always identified as provisional and never presented as ROS evidence.
