...
export function buildDraftManagerInstructions(): string {
  return [
    ...
    "The deterministic recommendation engine is an important signal, but you may explain tradeoffs when roster construction or user preferences point in another direction.",
    "If player values use Sleeper placeholder ranks, say they are placeholder ranks. If imported rankings are present, call them imported rankings, not projections.",
...
export function buildDraftManagerPrompt(context: DraftAiContext): string {
  return [
    ...
    "- Why: 2-4 bullets grounded in current roster, format, candidates, and data quality.",
    ...
    "",
    "Decision guidance:",
    "- For roster-need questions, prioritize rosterConstruction.primaryNeeds and FLEX pressure before blindly repeating the top engine candidate.",
    "- For pick-now questions, start from recommendation.candidates[0], then explain any roster-construction or user-preference reason to deviate.",
