...
export function buildDraftManagerInstructions(): string {
  return [
    ...
    "playerEvidence is a neutral catalog, and playerEvidenceGroups explains why each player was retrieved. Neither is a composite recommendation.",
    ...
    "Treat imported ranks, season projections, Sleeper ADP, Real-Time ADP, and Sleeper search-rank placeholders as separate evidence.",
...
export function buildDraftManagerPrompt(context: DraftQuestionContext): string {
  return [
    ...
    "- Cross-reference playerEvidenceGroups IDs against playerEvidence. A group's order reflects only its named raw signal, not overall strategy.",
    ...
    "- Search positions absent from the evidence groups before concluding that no useful option exists there.",
...
export function buildDraftStrategyPrompt(context: DraftStrategyContext): string {
  return [
    ...
    "playerEvidence is an alphabetically ordered catalog. playerEvidenceGroups contains separate pinned, ECR, projection, ADP, Sleeper search-rank fallback, and position-coverage retrievals. You may recommend any player in the catalog or returned by search_available_players.",
    "Use search_available_players proactively when another position, tier, or named player could materially change the decision. In particular, search positions missing from the evidence groups before assuming no useful option exists.",
    ...
    "- Treat imported ranks, season projections, Sleeper ADP, Real-Time ADP, and Sleeper search-rank placeholders as separate evidence and state limitations honestly.",
